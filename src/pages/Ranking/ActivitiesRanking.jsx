...
export default function ActivitiesRanking() {
  ...
  useEffect(() => {
    ...
    getUsers().then(async (responseUsers) => {
      getProjects().then(async (responseProjects) => {
          ...
          responseUsers.map(async (user) => {
            ...
            await Promise.all(
              responseProjects.map(async (project) => {
                await Promise.all(
                  project.registers.map(async (register, index) => {
                    if (
                      register.username === user.username &&
                      project.registers.findIndex(
        ...
        let rank = 0;
        let prevActivities = -1;
        setUsers(
          userData
            .sort((a, b) => b.activitiesTotal - a.activitiesTotal)
            .map((user, index) => {
              if (user.activitiesTotal != prevActivities) {
                rank = index + 1;
                prevActivities = user.activitiesTotal;
              }
              return { ...user, rank };
            })
        );
            ...
            >
              <div className="flex items-center gap-4 flex-1">
                ...
                <div>
                  ...
                  >
                    {user.activitiesTotal} hoạt động
                  </p>
                </div>
              </div>
